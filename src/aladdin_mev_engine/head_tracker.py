...
class EvmHead:
    ...
    def from_observation(cls, envelope: ObservationEnvelope) -> EvmHead:
        ...
        if set(payload) != expected:
            raise ValueError(
                "block-head payload fields do not match the governed contract"
            )
        return cls(
            ...
            block_timestamp_unix_s=parse_decimal(
                "block_timestamp_unix_s",
                payload["block_timestamp_unix_s"],
            ),
        )
...
class HeadTransition:
    ...
    def __post_init__(self) -> None:
        ...
        if contract.chain is not self.chain:
            raise ValueError(
                "head transition source contract does not match its chain"
            )
        require_u64("source_sequence", self.source_sequence)
        if (
            type(self.observation_kind) is not ObservationKind
            or self.observation_kind
            not in {
                ObservationKind.BLOCK_HEAD,
                ObservationKind.PRECONFIRMED_BLOCK,
            }
            ...
            raise ValueError(
                "head transition observation_kind is not a governed head kind"
            )
        if type(self.finality) is not Finality:
            raise TypeError(
                "head transition finality must be an exact Finality"
            )
        if not contract.permits(
            ...
            raise ValueError(
                "head transition stream is not permitted by its source contract"
            )
        ...
        _require_hash_or_none(
            "common_ancestor", self.common_ancestor, allow_zero=True
        )
        for name, values in (("removed", self.removed), ("added", self.added)):
            if type(values) is not tuple or any(
                ...
                raise TypeError(
                    f"{name} must be an exact tuple of non-zero canonical block hashes"
                )
        ...
        if self.common_ancestor in set(self.removed) | set(self.added):
            raise ValueError(
                "common ancestor cannot appear in a transition path"
            )
        ...
        if self.kind is HeadTransitionKind.BOOTSTRAP:
            ...
            branch_replacement = (
                ...
                and self.new_head != self.common_ancestor
            )
            pure_rollback = (
                not self.added and self.new_head == self.common_ancestor
            )
            if not (base_valid and (branch_replacement or pure_rollback)):
    ...
    def from_json_value(cls, value: object) -> HeadTransition:
        ...
        if any(type(value[field]) is not str for field in enum_fields):
            raise ValueError(
                "head transition governed enums must be exact JSON strings"
            )
        try:
            ...
            raise ValueError(
                "head transition contains an unknown governed enum"
            ) from error
    ...
    def verify_observation(self, envelope: ObservationEnvelope) -> None:
        ...
        if (
            ...
            raise ValueError(
                "head transition does not match its triggering observation"
            )
...
class HeadTracker:
    def __init__(
        ...
        *,
        ...
        if (
            type(observation_kind) is not ObservationKind
            or observation_kind
            not in {
                ObservationKind.BLOCK_HEAD,
                ObservationKind.PRECONFIRMED_BLOCK,
            ...
            raise ValueError(
                "HeadTracker requires a governed block-head observation kind"
            )
        ...
        if contract.chain is not chain:
            raise ValueError(
                "head tracker source contract does not match its chain"
            )
        if not contract.permits(
            ...
            raise ValueError(
                "source contract does not provide the governed full head stream"
            )
        self._chain = chain
    ...
    def current_head(self) -> EvmHead | None:
        return (
            ...
            else self._nodes[self._current_hash]
        )
    ...
    def apply(self, envelope: ObservationEnvelope) -> HeadTransition:
        ...
        if (
            ...
            raise ValueError(
                "head observation does not belong to this exact tracker stream"
            )
        head = EvmHead.from_observation(envelope)
        if (
            self._last_sequence is not None
            and head.source_sequence <= self._last_sequence
        ):
        ...
        if existing is not None and not existing.same_block_metadata(head):
            raise ValueError(
                "duplicate block hash carries conflicting block metadata"
            )

        if old_hash is None:
            if existing is not None or self._nodes:
                raise RuntimeError(
                    "head tracker authority is internally inconsistent"
                )
        ...
        parent = (
            self._nodes.get(head.parent_hash) if head.number > 0 else None
        )
        if parent is not None:
            if parent.number + 1 != head.number:
                raise ValueError(
                    "head number is inconsistent with its known parent"
                )
            if head.block_timestamp_unix_s < parent.block_timestamp_unix_s:
                raise ValueError(
                    "head timestamp moved backwards from its known parent"
                )
    ...
    def _node_for_candidate(
        self, block_hash: str, candidate: EvmHead
    ) -> EvmHead | None:
