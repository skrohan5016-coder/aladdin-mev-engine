...
LOCK_PATH = ROOT / "governance" / "f6-schemas.lock.json"
REQUIRED = (
...
def main() -> int:
    ...
    for name in REQUIRED:
        ...
        if not isinstance(identifier, str) or not identifier.startswith("https://schemas.aladdin.invalid/mev/"):
    ...
    try:
        ...
        errors.append(f"F6 schema lock is invalid or unreadable: {error}")
        lock = None
    if type(lock) is not dict or set(lock) != {"schema", "files"}:
        errors.append("F6 schema lock must be a closed object")
    else:
        if lock.get("schema") != "aladdin-mev-f6-schema-lock/v1":
            errors.append("F6 schema lock has an unknown schema")
        if lock.get("files") != digests:
            errors.append("F6 schema lock does not bind the exact schema digests")
    if errors:
        for error in errors:
