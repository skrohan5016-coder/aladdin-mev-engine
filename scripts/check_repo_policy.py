...
def workflow_policy_errors(text: str) -> list[str]:
    ...
    if text.count("persist-credentials: false") != 2:
        fail(
            errors,
            "both governed checkout paths must disable persisted credentials",
        )
    ...
    for line_number, line in enumerate(text.splitlines(), start=1):
        ...
        if revision.lower() != expected_revision:
            fail(
                errors,
                f"workflow action pin drift at line {line_number}: {action}",
            )
            continue
    ...
    for command, expected_count in EXPECTED_RUN_COMMANDS.items():
        ...
        if actual_count != expected_count:
            fail(
                errors,
                f"workflow must use governed run command exactly "
...
def check_workflow(errors: list[str]) -> None:
    ...
    if not workflow.is_file():
        ...
        return
    errors.extend(
        workflow_policy_errors(workflow.read_text(encoding="utf-8"))
    )
...
def check_source(errors: list[str]) -> None:
    for base in (ROOT / "src", ROOT / "scripts"):
        for path in sorted(base.rglob("*.py")):
            ...
            if (
                path.name != "check_repo_policy.py"
                and FORBIDDEN_SOURCE_TERMS.search(text)
            ):
...
def check_repository_hygiene(errors: list[str]) -> None:
    ...
    if missing:
        fail(
            errors,
            "missing credential ignore rules: "
            + ", ".join(sorted(missing)),
        )
    ...
    for path in sorted(ROOT.rglob("*")):
        ...
        if path.is_symlink():
            fail(
                errors,
                f"repository symlink is forbidden: {path.relative_to(ROOT)}",
            )
            continue
        if not path.is_file():
            continue
        if path.stat().st_size > 1_048_576:
            fail(
                errors,
                f"repository file exceeds 1 MiB: {path.relative_to(ROOT)}",
            )
        data = path.read_bytes()
        if b"\x00" in data:
            fail(
                errors,
                f"binary/NUL content is forbidden: {path.relative_to(ROOT)}",
            )
            continue
        try:
            ...
            fail(
                errors,
                f"non-UTF-8 file is forbidden: {path.relative_to(ROOT)}",
            )
            continue
        if PRIVATE_MATERIAL.search(text):
            fail(
                errors,
                f"possible private material in {path.relative_to(ROOT)}",
            )
        for line_number, line in enumerate(text.splitlines(), start=1):
            if line.rstrip(" \t") != line:
                fail(
                    errors,
                    f"trailing whitespace in {path.relative_to(ROOT)}:"
                    f"{line_number}",
                )
...
def load_source_contract_file(
    ...
    if type(contracts) is not list or len(contracts) != len(
        ...
        fail(
            errors,
            "source-contract registry must contain exactly nine contracts",
        )
        return document
    source_ids = [
        contract.get("source_id") if type(contract) is dict else None
        for contract in contracts
    ]
    if tuple(source_ids) != EXPECTED_SOURCE_IDS:
        fail(
            errors,
            "source-contract registry identifiers are not the governed sorted set",
        )
    digest = document.get("contracts_sha256")
    if type(digest) is not str or SHA256.fullmatch(digest) is None:
        fail(
            errors,
            "source-contract registry digest is not canonical SHA-256",
        )
    elif digest != _canonical_sha256(contracts):
        fail(
            errors,
            "source-contract registry digest does not bind its contract vector",
        )
    return document
...
def check_architecture(
    ...
    for key, expected in EXPECTED_PARENT.items():
        if document.get(key) != expected:
            fail(
                errors,
                f"architecture {key} does not bind the accepted F0 parent",
            )
    for key in (
        "execution_authority",
        "network_access",
    ...
    if document.get("observation_authority") != "recorded-input-only":
        fail(
            errors,
            "architecture observation_authority must be recorded-input-only",
        )
    ...
    if tuple(document.get("source_contract_ids", ())) != EXPECTED_SOURCE_IDS:
        fail(
            errors,
            "architecture source-contract identifiers do not match the registry",
        )
    components = document.get("components")
    if type(components) is not list or not REQUIRED_F1_COMPONENTS.issubset(
        set(components)
    ):
        fail(errors, "architecture is missing required F1 components")
    invariants = document.get("invariants")
    if type(invariants) is not list or not REQUIRED_F1_INVARIANTS.issubset(
        set(invariants)
    ):
    ...
    if source_contracts is not None and (
        ...
        fail(
            errors,
            "architecture source-contract digest does not match the governed registry",
        )
