...
LOCK_PATH = ROOT / "governance" / "f3-schemas.lock.json"
REQUIRED = (
...
def main() -> int:
    ...
    for name in REQUIRED:
        ...
        if (
            ...
            or not identifier.startswith("https://schemas.aladdin.invalid/mev/")
        ):
        ...
        for reference in _references(value):
            ...
            if not relative or not (SCHEMA_DIR / relative).is_file():
                errors.append(
                    f"{name}: unresolved relative schema reference {reference}"
                )
    ...
    try:
        ...
        errors.append(f"F3 schema lock is invalid or unreadable: {error}")
        lock = None
    if type(lock) is not dict or set(lock) != {"schema", "files"}:
        errors.append("F3 schema lock must be a closed object")
    else:
        if lock.get("schema") != "aladdin-mev-f3-schema-lock/v1":
            errors.append("F3 schema lock has an unknown schema")
        if lock.get("files") != digests:
            errors.append("F3 schema lock does not bind the exact schema digests")
