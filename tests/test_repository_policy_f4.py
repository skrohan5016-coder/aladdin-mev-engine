...
class RepositoryWorkflowPolicyF4Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.workflow = (ROOT / ".github" / "workflows" / "ci.yml").read_text(encoding="utf-8")

    def test_governed_f4_workflow_is_accepted(self) -> None:
        self.assertEqual(workflow_policy_errors(self.workflow), [])

    def test_f4_schema_verification_is_mandatory(self) -> None:
        mutated = self.workflow.replace("python scripts/verify_f4_schemas.py", "python -V", 1)
        errors = workflow_policy_errors(mutated)
        self.assertTrue(any("run command is not allowlisted" in error for error in errors))
        self.assertTrue(any("verify_f4_schemas.py" in error for error in errors))

    def test_source_and_merge_identities_are_mandatory(self) -> None:
        source = self.workflow.replace("name: F4 exact-head conformance", "name: generic", 1)
        merge = self.workflow.replace("name: F4 merge integration", "name: generic", 1)
        self.assertTrue(any("F4 exact-head" in error for error in workflow_policy_errors(source)))
        self.assertTrue(any("F4 merge-integration" in error for error in workflow_policy_errors(merge)))

    def test_execution_wrapper_bypasses_are_rejected(self) -> None:
        mutations = (
            self.workflow + "\ndefaults:\n  run:\n    shell: python {0}\n",
