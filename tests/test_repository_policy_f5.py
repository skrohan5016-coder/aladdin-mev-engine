...
class RepositoryWorkflowPolicyF5Tests(unittest.TestCase):
    ...
    def test_governed_f5_workflow_is_accepted(self) -> None:
    ...
    def test_source_and_merge_identities_are_mandatory(self) -> None:
        source = self.workflow.replace("name: F5 exact-head conformance", "name: generic", 1)
        merge = self.workflow.replace("name: F5 merge integration", "name: generic", 1)
        self.assertTrue(any("F5 exact-head" in error for error in workflow_policy_errors(source)))
        self.assertTrue(any("F5 merge-integration" in error for error in workflow_policy_errors(merge)))
